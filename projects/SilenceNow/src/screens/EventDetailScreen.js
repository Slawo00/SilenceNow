...
export default function EventDetailScreen({ route, navigation }) {
  ...
  const handleDeleteWitness = (witness) => {
    Alert.alert(
      'Zeuge entfernen',
      `"${witness.witness_name}" wirklich entfernen?`,
      [
    ...
    <View style={styles.container}>
      <ScrollView style={styles.scrollView}>
        ...
        <View style={styles.section}>
          ...
          {witnesses.length === 0 ? (
            <Text style={styles.emptyHint}>
              Keine Zeugen dokumentiert. Zeugen stärken deine Rechtsposition erheblich!
            </Text>
          ) : (
            witnesses.map((w) => (
              <TouchableOpacity
                ...
                onLongPress={() => handleDeleteWitness(w)}
              >
                <View style={styles.witnessHeader}>
                  <Text style={styles.witnessName}>👤 {w.witness_name}</Text>
                  {w.witness_relationship && (
                    <Text style={styles.witnessRelation}>{w.witness_relationship}</Text>
                  )}
                </View>
                {w.witness_contact && (
                  <Text style={styles.witnessContact}>📱 {w.witness_contact}</Text>
                )}
                {w.statement && (
        ...
        <View style={styles.section}>
          ...
          {notes.length === 0 ? (
            <Text style={styles.emptyHint}>
              Keine Notizen vorhanden. Beschreibe was du gehört/erlebt hast!
            </Text>
          ) : (
            notes.map((n) => (
              <TouchableOpacity
              ...
              >
                <View style={styles.noteHeader}>
                  <Text style={styles.noteType}>
                    {noteTypeLabels[n.note_type] || '📝 Allgemein'}
                  </Text>
...
const styles = StyleSheet.create({
  ...
  witnessCard: {
    ...
    marginBottom: 8,
    borderLeftWidth: 3,
    borderLeftColor: COLORS.ELECTRIC_GREEN,
  },
  witnessHeader: {
    flexDirection: 'row',
    justifyContent: 'space-between',
